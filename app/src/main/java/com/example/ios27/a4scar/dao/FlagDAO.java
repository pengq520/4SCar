...
public class FlagDAO {
    ...
    public void add(Tb_flag tb_flag){
        ...
        db.execSQL("insert into tb_flag(_id,flag) values(?,?)",
                new Object[]{tb_flag.getid(),tb_flag.getFlag()});// 执行添加备忘录信息操作
    }
    ...
    public Tb_flag find(int id) {
        ...
        Cursor cursor=db.rawQuery("select _id,flag from tb_flag where _id=?",
                new String[]{String.valueOf(id)});// 根据编号查找备忘录信息，并存储到Cursor类中
        if (cursor.moveToNext()){// 遍历查找到的备忘录信息

            // 将遍历到的备忘录信息存储到Tb_flag类中
    ...
    public List<Tb_flag> getScrollData(int start,int count){
        ...
        while (cursor.moveToNext()) {// 遍历所有的备忘录信息
    ...
    public long getCount(){
        db=helper.getWritableDatabase();// 初始化SQLiteDatabase对象
        Cursor cursor=db.rawQuery("select count(_id) from tb_flag",null);// 获取备忘录信息的记录数
    ...
    public int getMaxId(){
        db=helper.getWritableDatabase();// 初始化SQLiteDatabase对象
        Cursor cursor=db.rawQuery("select max(_id) from tb_flag",null);// 获取备忘录信息表中的最大编号
